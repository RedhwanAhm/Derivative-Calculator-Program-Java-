...
public class quotientRule
{
  ...
  public quotientRule (String s)
  {
    ...
    for (int i = 0; i < p1.size(); i++) //removes '(' and ')'
    {
      String a = p1.get(i);
  ...
  public void Combine (ArrayList <String> arr, ArrayList <String> arr2)
  {
    ...
    while (i <= 4)
    {
     if (i == 0) //compiles result, adds in low d high
     {
       reslt.add(temp.get(0));
     }
     if (i == 1)//compiles result, adds in '-'
     {
      reslt.add("-"); 
     }
     if (i == 2)//compiles result, adds in high d low
     {
       reslt.add(temp.get(1));
     }
     if (i==3)//compiles result, adds in the division symbol
     {
       reslt.add("/"); 
     }
     if(i==4)//compiles result, adds in low^2
     {
      reslt.add("("+arr.get(1)+")^2"); 
     }
     i++;
    }
  }
